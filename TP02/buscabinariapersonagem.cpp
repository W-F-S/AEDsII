...
//metodo de busca binaria usando a chave nome

int binarySearch(Lista *lista, int esq, int dir,  char* buscado, int cont)
{	
	if(esq <= dir)
	{
		int index = (esq + dir) / 2; //posicao central da porcao do vetor em que a busca esta sendo efetuada
		//a ideia e ir dividindo a lista ao meio ate que o elemento seja encontrado ou que a liste acabe
		if(strcmp(lista->lista[index]->nome, buscado) == 0)
		{
			cont+=1; //contador do numero de comparacoes sendo feitas
			printf("SIM\n");
		}
		else if(strcmp(buscado, lista->lista[index]->nome) < 0)
		{
			cont+=2;
			cont += binarySearch(lista, esq, index - 1, buscado, cont);
		}
		else
		{
			cont+=2;
			cont += binarySearch(lista, index + 1, dir, buscado, cont);
		}
	}
	else
	{
...
int main()
{
	...
	while(!isFim(entrada))
	{
		//buscar personagem na lista
		comparacoes = binarySearch(lista, 0, lista->n, entrada, 0);
		fgets(entrada, 1000, stdin);
		entrada[strlen(entrada)-1] = '\0';
	}
	clock_t fim = clock();
	...
	FILE *file = fopen("680715_binaria.txt", "wt");
	fprintf(file, "Matricula: 680715;\t %d comparacoes;\t tempo de pesquisa total: %lfs\t", comparacoes, total);
        fclose(file);	
