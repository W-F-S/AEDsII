...
void swap(Lista *lista, int i, int j)
{
	...
	//troca de posicao o personagem com o auxilio de uma variavel temporaria e de acordo com as posicoes passadas por parametro
...
{
	//insercao dos elementos de acordo com o valor de h
	for(int i = (h + cor); i < lista->n; i += h)
	{
		//variavel temporaria para efetuar trocas de posicao
		Personagem *temp = (Personagem*) malloc (sizeof(Personagem));
		ConstVazio(temp);
		temp = clone(lista->lista[i]);
		int j = i - h;
		//comparar elementos de acordo com o peso do personagem e desloca-los
		while((j >= 0) && (lista->lista[j]->peso > temp->peso))
		{
			lista->lista[j+h] = clone(lista->lista[j]);
			j = j - h;
		}
		lista->lista[j + h] = temp;
	}
}
void insercaoDesempate(Lista *lista)
{
	//realiza a insercao uma ultima vez, fazendo o desempate dos personagens que tem pesos iguais a partir do nome
	for(int i = 1; i < lista->n; i++)
	{
		//alocacao de uma variavel temporaria para efetuar as comparacoes
		Personagem *temp = (Personagem*) malloc (sizeof(Personagem));
		ConstVazio(temp);
		temp = lista->lista[i];
		int j = i - 1;
		//desempate pelo nome e deslocamento dos elementos na lista
		while((j >= 0) && (lista->lista[j]->peso == temp->peso && strcmp(lista->lista[j]->nome, temp->nome) > 0))
		{
			lista->lista[j+1] = clone(lista->lista[j]);
			j--;
		}
		lista->lista[j+1] = temp;
	}
}
void shellSort(Lista *lista, int log[])
{
	//declarar variaveis
	int h = 1;
	//estabelecer a sequencia para o h de acordo com o tamanho da lista
	//a sequencia corresponde a 1, 4, 13, 40, 121 (mas o 121 vira 40 ao entrar no outro loop
	do
	{
		h = (h * 3) + 1;
	}while(h < lista->n);
	//realizar as insercoes dos elementos na lista
	while(h > 0)
	{
	...
	//realizar o desempate por nome
	insercaoDesempate(lista);
}
...
int main()
{
	...
	shellSort(lista, contadores); //j = i + 1
	clock_t fim = clock();
	...
	FILE *file = fopen("680715_selecaoRecursiva.txt", "wt");
