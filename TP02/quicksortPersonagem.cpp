...
void quickSort(Lista *lista, int esquerda, int direita, int *cont)
{
	//declarar variaveis
	Personagem *pivo = (Personagem*) malloc (sizeof(Personagem));
	pivo = lista->lista[(direita + esquerda)/2]; 
	int i = esquerda;
	int j = direita;

	while(i <= j)
	{
		while(((strcmp(lista->lista[i]->corDoCabelo, pivo->corDoCabelo) < 0)) || ((strcmp(lista->lista[i]->corDoCabelo, pivo->corDoCabelo)== 0 && strcmp(lista->lista[i]->nome, pivo->nome) < 0))) //do lado esquerdo ficam os elementos menores que o pivo

		{
			i++;
			cont[0] += 2;//paga as duas comparacoes no pior caso
		}

		while((strcmp(lista->lista[j]->corDoCabelo, pivo->corDoCabelo) > 0) || (strcmp(lista->lista[j]->corDoCabelo, pivo->corDoCabelo) == 0 && strcmp(lista->lista[j]->nome, pivo->nome) > 0))//do lado direito ficam os elementos maiores que o pivo
	...
	//continuar biparticionando a lista e ordenando
	if(esquerda < j)
	{
		quickSort(lista, esquerda, j, cont);
	}
	if(i < direita)
	{
		quickSort(lista, i, direita, cont);
	}
}
//----------------------------------------------------------
...
int main()
{
	...
	quickSort(lista, 0, lista->n - 1, contadores); //j = i + 1
	clock_t fim = clock();
