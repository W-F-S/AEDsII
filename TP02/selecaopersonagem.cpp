...
void selectionSort(Lista *lista, int i, int j, int menor, int log[])
{
	//o ultimo elemento da lista estara ordenado ao final, logo o i nao precisa ir ate n, somente ate n - 1
	if(i < lista->n - 1)
	{
		//caminhar toda a lista comparando cada elemento ate achar o menor
		if(j < lista->n)
		{
			
			//se encontrar um elemento menor que o atual menor, o menor ate entao recebe a posicao do menor encontrado
			if(strcmp(lista->lista[j]->nome, lista->lista[menor]->nome) < 0)
			{
				menor = j;
			}
			log[0]++;
			selectionSort(lista, i, j + 1, menor, log);
		 }
		 else
		 {	 
			//realizar a troca de posicao dos elementos na lista 
			swap(lista, i, menor);
			log[1]++;
			menor = i + 1;
			selectionSort(lista, i + 1, i + 2, menor, log);
		 }	
	}
}
//----------------------------------------------------------
...
int main()
{
	...
	selectionSort(lista, 0, 1, 0, contadores); //j = i + 1
	clock_t fim = clock();
