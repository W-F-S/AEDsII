...
class ListaSequencial extends Personagem
{
	...
	public void MergeSort(ListaSequencial aux, int inicio, int fim, int[] cont) throws Exception
	{
		if(inicio < fim)
		{
			int meio = (inicio + fim)/2;
			//subdividir os arrays recursivamente ate que o subarray fique com tamanho 1, e depois combinar os elementos ordenando-os
			MergeSort(aux, inicio, meio, cont);
			MergeSort(aux, meio + 1, fim, cont);
			combinacao(aux, inicio, meio, fim, cont);
		}
		//realizar o desempate dos personagens com o mesmo planeta de origem pelo nome
		//para isso sera usado o algoritmo de insertion sort que é considerado bala de prata quando o arranjo ja esta em ordem crescente
		desempate(cont);
	}
	public void combinacao(ListaSequencial aux, int inicio, int meio, int fim, int[] cont) throws Exception
	{
		//declarar variaveis
		int j = inicio; //para caminhar na primeira metade da lista
		int k = meio + 1; //para caminhar na segunda metade da lista
		//copiar elementos da lista para a lista auxiliar
		for(int i = inicio; i <= fim; i++)
		{
			aux.lista[i] = lista[i];
			cont[1]++; //contador de movimentacoes de elementos do array
		}
		
		//combinar os elementos ordenando-os
		for(int i = inicio; i <= fim; i++)
		{
			//se a primeira metade do vetor terminar primeiro, copia o restante dos elementos da segunda metade
			if(j > meio)
			{
				this.lista[i] = aux.lista[k++];
				cont[1]++;
			}
			//se a segunda metade terminar primeiro, copiar para o vetor o restante dos elementos da primeira metade
			else if(k > fim)
			{
				this.lista[i] = aux.lista[j++];
				cont[1]++;
			}
			//se o elemento da posicao i tiver homeworld 'menor' que o da posicao j
			else if(aux.lista[j].getHomeworld().compareTo(aux.lista[k].getHomeworld()) < 0)
			{
				this.lista[i] = aux.lista[j++];
				cont[0]++; //contador de comparacoes entre elementos do array
				cont[1]++;
			}
			else //se nenhum caso acima ocorreu
			{
				this.lista[i] = aux.lista[k++];
				cont[1]++;
			}
	...
	public void desempate(int[] contadores)
	{
		//declarar variaveis
		Personagem temp = new Personagem();
		int j = 0;
		for(int i = 1; i < n; i++) //i comeca em um pois entende-se que o primeiro elemento da lista ja esta ordenado 
		{
			temp = lista[i];
			contadores[1]++; //contador de movimentacoes de elementos do array
			j = i - 1; //portanto, j comeca sempre no ultimo elemento da parte ordenada da lista
			while((j >= 0) && (lista[j].getHomeworld().compareTo(temp.getHomeworld()) == 0 && lista[j].getNome().compareTo(temp.getNome()) > 0)) //enquanto os elementos da parte ordenada forem maiores que o armazenado em temp, estes serao deslocados para a direita ate que encontremos a posicao certa para o elemento de temp (da posicao i)
			{
				lista[j + 1] = lista[j];
				contadores[1]++;
				j--; //a primeira condicao do laco evita que j seja negativo e, portanto, uma posicao invalida a ser acessada
			}
			lista[j + 1] = temp; //quando o elemento da posicao j for menor que temp, temp sera colocada na posicao em seguida (j + 1)
			contadores[1]++;
			contadores[0]++; //contador de comparacoes entre elementos do array
		}
	}
}

public class MergeSortPersonagem
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		while(!isFim(entrada))
		{
			...
			ListaSequencial auxiliar = new ListaSequencial(100);
			int[] cont = new int[2];
			long inicio = now();
			lista.MergeSort(auxiliar, 0, lista.getN()-1, cont);
			long fim = now();
			//imprimir a lista
			lista.imprimirLista();
			//gravar arquivo de log
			Arq.openWrite("680715_mergesort.txt");
