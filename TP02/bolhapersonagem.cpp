...
int* bubbleSort(Lista *lista)
{
	...
	{	
		...
		{
			if(strcmp(lista->lista[i + 1]->anoNascimento, lista->lista[i]->anoNascimento) < 0) //compara dois a dois por vez, trocando de lugar quando o primeiro for menor que o segundo
			{
				ordenado = false;
				swap(lista, i, i + 1);
				contadores[1]++; //contador de n de movimentacoes entre elementos do array
			}
			contadores[0]++;//contador do numero de comparacoes entre elementos do array
		}
	}while(ordenado == false); //repetir ate que nenhuma troca seja feita
	return contadores;
}
//----------------------------------------------------------
...
int main()
{
	...
	contadores = bubbleSort(lista); //j = i + 1
	clock_t fim = clock();
	...
	FILE *file = fopen("680715_bolha.txt", "wt");
