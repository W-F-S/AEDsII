...
class ListaSequencial extends Personagem
{
	...
	public int[] selectionSortParcial(int k) throws Exception
	{
		//declarar variaveis
		int menor = 0;
		int x = 0;
		int[] contadores = new int[2];
		for(int i = 1; i <= k; i++) // o algoritmo roda ate k, incluindo-o, representando uma ordenacao parcial dos k primeiros elementos
		{
			menor = i;
			for(x = i + 1; x < n-1; x++) //comparar um a um. A cada vez que o for interno termina e o swap e feito, a parte ordenada aumenta. Por isso x e igual a i + 1;
			{
				if(lista[x].getNome().compareTo(lista[menor].getNome()) < 0)
				{
					menor = x;
				}
				contadores[0]++;
			}
			swap(menor, i); //efetua a troca de posicao entre o menor elemento e o primeiro elemento da parte nao ordenada
			contadores[1]++;
		}
		return contadores;
	}
...
public class PartialSelectionSortPersonagem
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		while(!isFim(entrada))
		{
			...
			int k = 10;
			long inicio = now();
			contadores = lista.selectionSortParcial(k);
			long fim = now();
			//imprimir a lista
			lista.imprimirLista(10);

			//gravar arquivo de log
			Arq.openWrite("680715_selecaoparcial.txt");
