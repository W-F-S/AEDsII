...
class ListaSequencial extends Personagem
{
	...
	public int[] inserctionSort()
	{
		...
		for(int i = 1; i < n; i++) //i comeca em um pois entende-se que o primeiro elemento da lista ja esta ordenado 
		{
			...
			while(j >= 0 && lista[j].getAnoNascimento().compareTo(temp.getAnoNascimento()) > 0) //enquanto os elementos da parte ordenada forem maiores que o armazenado em temp, estes serao deslocados para a direita ate que encontremos a posicao certa para o elemento de temp (da posicao i)
			{
				lista[j + 1] = lista[j];
				contadores[1]++;
				contadores[0]++;
...
public class InserctionSortPersonagem
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		while(!isFim(entrada))
		{
			...
			int[] cont = new int[2];
			long inicio = now();
			cont = lista.inserctionSort();
			long fim = now();
			//imprimir a lista
			lista.imprimirLista();
			//gravar arquivo de log
			Arq.openWrite("680715_insercao.txt");
