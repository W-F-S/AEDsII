...
class Fila
{
	...
	//metodo para saber o tamanho da fila
	public int tamanhoFila()
	{
		int tamanho = 0;
		if(primeiro != ultimo)
		{
			for(Celula i = primeiro.prox; i != ultimo.prox; tamanho++, i = i.prox);
		}
		return tamanho;
	}
	//metodo para inserir elementos na fila, sempre no fim
	public void inserir(Personagem sw) throws Exception
	{
		if(tamanhoFila() < 5)
		{
			ultimo.prox = new Celula(sw); //aloca celula para novo elemento
			ultimo = ultimo.prox; //move o ponteiro do ultimo
			ultimo.prox = primeiro; //liga o ultimo.prox no primeiro por se tratar de uma fila circular
		}
		//caso a fila esteja cheia, remover um elemento antes de inserir outro
		else
		{
			remover();
			inserir(sw);
	...
	//metodo para remover elementos da fila, sempre do inicio
	public Personagem remover() throws Exception
	{
		...
		primeiro = temp.prox;
		Personagem removido = primeiro.elemento;
		temp.prox = null;
		temp = null;
		return removido;
	}
	//metodo para mostrar os elementos presentes na fila
	...
	//metodo para calcular a media das alturas dos personagens presentes na fila e fazer o display dessa media na tela
	public void imprimirMedia()
	{
		...
		int resp = 0;
		double cont = 0.0;
		//somar as alturas de todos os personagens contidos na fila e entao fazer a media dessas alturas
	...
	public static void main(String[] args) throws Exception
	{
		...
		while(!isFim(entrada))
		{
			...
			for(int j = 0; j < nOperacoes; j++)
			{
				...
				//determinar qual operacao devera ser feita e realiza-la (insercao ou remocao)
				...
				else if(operacao[0].charAt(0) == 'R')
				{
					...
					starWars = fila.remover();
					removidos = starWars.getNome();
					MyIO.println("(R) " + removidos);
					x++;	
				}
			}
			//imprimir fila se nao estiver vazia
			fila.imprimirFila();
	}
}
