...
class Pilha extends Personagem
{
	//atributos
	...
	{
		pilha = new Personagem[tamanho];
		n = 0;
	}

	//metodo para empilhar (inserir elemento)
	public void empilhar(Personagem personagem) throws Exception
	{
		//verificar se a pilha esta cheia
		if(n >= pilha.length)
		{
			throw new Exception("A pilha esta cheia, nao ha como inserir mais elementos.");
		}
		//inserir elemento
		pilha[n] = personagem;
		n++;
	}
	//metodo para desempilhar (remover elemento da pilha)
	public Personagem desempilhar() throws Exception
	{
		...
		//remover elemento
		personagem = clone(pilha[n-1]);
		n--;
		return(personagem);
	}
	//metodo para printar a pilha 
	public void mostrar()
	{
		for(int i = 0; i < n; i++)
		{
...
public class PilhaPersonagem
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		while(!isFim(entrada))
		{
			...
			starWars.ler(entrada); //ler os atributos do personagem
			pilha.empilhar(starWars); //empilhar o personagem
			//receber nova entrada
			entrada = MyIO.readLine();
		}

		//receber demais entradas
		...
		for(int i = 0; i < nOperacoes; i++)
		{
			...
			entrada = MyIO.readLine();
			entrada = starWars.changeEncodingISOtoUTF(entrada);
			//separar a string recebida em 3 partes
			aux = entrada.split(" ");
			//verificar a operacao a ser feita
			if(aux[0].charAt(0) == 'I') //empilhar/inserir
			{
				starWars.ler(aux[1]);
				pilha.empilhar(starWars);
			}
			else //desempilhar
			{
				starWars = pilha.desempilhar();
