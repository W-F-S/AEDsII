...
//metodo para encontrar o maior altura no array
int getMaior(Lista *lista, int log[])
{
	int maior = 0;
	for(int i = 0; i < lista->n; i++)
	{
		if(maior < lista->lista[i]->altura)
		{
			maior = lista->lista[i]->altura;
			log[1]++; //movimentacoes
		}
		log[0]++; //comparacoes
	}
...
//metodo para encontrar o numero de digitos em um numero inteiro
void counting(Lista *lista, int exp, int log[])
{
	...
	for(int i = 0; i < lista->n; i++)
	{
		//int valor = (lista->lista[i]->altura/exp) % 10;
		contagem[(lista->lista[i]->altura/exp) % 10]++;
	}

	//tornar array de contagem acumulativo
	for(int i = 1; i < 10; i++)
	{
	...
	{
		int posicao = contagem[(lista->lista[i]->altura/exp) % 10] - 1;
		//a lista na posicao ditada pelo arranjo de contagem, que contem a posicao que cada elemento deve ficar, recebe o elemento da lista original
		ordenada->lista[posicao] = clone(lista->lista[i]);
		log[1]++; //contabiliza o numero de movimentacoes de elementos da lista
		contagem[((lista->lista[i]->altura/exp)%10)]--;
	}
	//copiar a lista ordenada para a lista original 
	for(int i = 0; i < lista->n; i++)
	{
		lista->lista[i] = clone(ordenada->lista[i]);
		log[1]++;
	}
}
void RadixSort(Lista *lista, int log[])
{
	//encontrar o maior valor no array
	int maior = getMaior(lista, log);

	//chamar metodo do counting para executar a ordenacao para cada digito do numero. Ao chamar o counting é passado o expoente como parametro, que corresponde ao digito do numero(i) elevado a 10
	for(int exp = 1; maior/exp > 0; exp *= 10)
	{
		counting(lista, exp, log);
	}
}
//----------------------------------------------------------
...
int main()
{
	...
	RadixSort(lista, contadores); //j = i + 1
	clock_t fim = clock();
	...
	FILE *file = fopen("680715_radixsort.txt", "wt");
