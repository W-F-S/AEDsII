...
class ListaSequencial extends Personagem
{
	...
	{
		//declarar variaveis
		int maior = Math.round((float)lista[0].getPeso());
		int menor = Math.round((float)lista[0].getPeso());
		ListaSequencial ordenada = new ListaSequencial(n);;
		int[] cont = new int[2];
		//descobrir o maior e o menor peso
		for(int i = 1; i < n; i++)
		{
			if(Math.round(lista[i].getPeso()) > maior)
			{
				cont[0]++;
				maior = Math.round((float)lista[i].getPeso());
				cont[1]++; //contador do numero de movimentacoes de elementos do array
			}
			else if(Math.round(lista[i].getPeso()) < menor)
			{
				cont[0] += 2;//contador de numero de comparacoes entre elementos do array
				menor = Math.round((float)lista[i].getPeso());
				cont[1]++;
			}
			else
			{
		...
		//contar quantos elementos tem de cada
		for(int i = 0; i < n; i++)
		{
			posicao = Math.round((float)lista[i].getPeso());
			contagem[posicao]++;
			cont[1]++;
		}

		//tornar o array de contagem acumulativo
		...
		for(int i = n -1; i >= 0; i--)
		{
	 		posicao = contagem[Math.round((float)lista[i].getPeso())] - 1;
			ordenada.lista[posicao] = this.lista[i];
			cont[1]++;
			contagem[Math.round((float)lista[i].getPeso())]--;
		}
...
public class CountingSortPersonagem
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		while(!isFim(entrada))
		{
			...
			long inicio = now();
			contadores = lista.countingSort();
			long fim = now();
			//imprimir a lista
			//lista.imprimirLista();

			//gravar arquivo de log
			Arq.openWrite("680715_countingsort.txt");
