...
class ListaSequencial extends Personagem
{
	...
	public int[] selectionSort() throws Exception
	{
		...
		for(int i = 0; i < n - 1; i++) // o algoritmo roda ate length -1 pois ao final, o ultimo elemento ja estar na posicao correta
		{
			menor = i;
			for(x = i + 1; x < n; x++) //comparar um a um. A cada vez que o for interno termina e o swap e feito, a parte ordenada aumenta. Por isso x e igual a i + 1;
			{
...
public class SelectionSortPersonagem
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		while(!isFim(entrada))
		{
			...
			int[] contadores = new int[2];
			long inicio = now();
			contadores = lista.selectionSort();
			long fim = now();
			//imprimir a lista
			lista.imprimirLista();

			//gravar arquivo de log
			Arq.openWrite("680715_selecao.txt");
