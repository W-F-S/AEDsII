...
class ListaSequencial extends Personagem
{
	...
	//metodo de pesquisa sequencial utilizando o atributo nome
	public int pesquisarPorNome(String nome)
	{
		...
		for(int i = 0; i < n; i++)
		{
			name = lista[i].getNome();
			if(name.equals(nome))
			{
				resposta = true;
			}
			cont++; //contador do numero de comparacoes feitas pelo algoritmo de busca
		}
		//verificar se o elemento buscado foi encontrado e printar a resposta
		if(resposta == true)
		{
			MyIO.println("SIM");
		}
...
public class PesquisaSequencialPersonagem
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		while(!isFim(nome))
		{
			//pesquisar se o personagem se encontra na lista
			cont = lista.pesquisarPorNome(nome);
			nome = MyIO.readLine();
		}
		//tempo final
		fim = now();
		...
		Arq.openWrite("680715_sequencial.txt");
		Arq.print("Matricula : 680715 \t" + "Por pesquisa: " + cont + " comparacoes\t" + "Tempo de Pesquisa total: " + (fim - inicio) + "ms\t");	
		Arq.close();
	}
}
