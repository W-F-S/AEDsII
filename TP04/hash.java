...
class Hash
{
	...
	private int tamTab, tamRes, posRes;
	private Personagem[] reserva;
	...
	public Hash()
	{
		...
		this.reserva = new Personagem[tamRes];
		for(int i = 0; i < tamTab; tabela[i++] = null);
		for(int i = 0; i < tamRes; reserva[i++] = null);	
		this.posRes = 0;
	}
	public int hash(Personagem x)
	{
	...
	public void inserir(Personagem x)
	{
		...
		else if(posRes < tamRes)
		{
			reserva[posRes++] = x;
		}
	}
	public boolean pesquisar(String x, int[] cont)
	{
		boolean resp = false;
		for(int i = 0; i < tamTab; i++)
		{
			if(tabela[i] != null && tabela[i].getNome().compareTo(x) == 0)
			{
				resp = true;
				i = tamTab;
			}
		...
		if(!resp)
		{
			...
			{
				if(reserva[i] != null && reserva[i].getNome().compareTo(x) == 0)
				{
					resp = true;
					i = tamRes;
				}
...
public class hash
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		while(!isFim(entrada))
		{
			boolean resp = hash.pesquisar(entrada, cont);
		...
		Arq.openWrite("680715_hashReserva.txt");
